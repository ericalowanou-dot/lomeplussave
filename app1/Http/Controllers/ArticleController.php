...
class ArticleController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $articlesQuery = Article::query();
         ...
         // 🔹 Filtrer par sous-catégorie spécifique (prioritaire sur catégorie)
        if ($request->filled('sous_categorie')) {
            $articlesQuery->where('sous_categorie_id', $request->sous_categorie);
        }
        // 🔹 Filtrer par catégorie (via sous-catégorie) - avec cache
        ...
        // 🔹 Nombre d'articles par page (12, 24, 48, 96)
    ...
    public function store(Request $request){
        ...
        foreach ($photos as $idx => $photo) {
            ...
            // Vérifier la taille (5 Mo = 5120 Ko)
            if ($photo->getSize() > 5120 * 1024) {
                return back()
                    ->withErrors(['photos' => 'Une ou plusieurs photos dépassent la taille maximale de 5 Mo.'])
                    ->withInput();
            }
        }
        ...
        try {
            ...
            if (str_contains($exception->getMessage(), 'permission') || str_contains($exception->getMessage(), 'writable')) {
                $errorMessage = 'Erreur de permissions : le dossier de destination n\'est pas accessible en écriture.';
            } elseif (str_contains($exception->getMessage(), 'read') || str_contains($exception->getMessage(), 'Invalid')) {
    ...
    public function update(Request $request, Article $article)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'photos.*' => 'image|mimes:jpeg,png,jpg,gif,webp,bmp,heic,heif,svg,avif|max:5120',
            ], [
                'categorie.required' => 'La catégorie est obligatoire.',
                'categorie.exists' => 'La catégorie sélectionnée n\'existe pas.',
                'sous_categorie_id.required' => 'La sous-catégorie est obligatoire.',
